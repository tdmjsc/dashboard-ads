...
const USERS = [
  ...
  { user: 'truongphong1', pass: '123456', role: 'viewer', employees: ['Phương', 'My', 'Huân'] },

  // Ví dụ nhân viên — chỉ xem chính mình:
  { user: 'phuong', pass: '123456', role: 'viewer', employees: ['Phương'] },
];
...
async function fetchAccount(acc, token, days, since, until) {
  ...
  for (const c of camps) {
    byId[c.id] = {
      ...
      obj: 'kết quả',
      link: null,                            // ← link bài quảng cáo (lấy ở cấp ad)
      adsets: [],                            // ← các nhóm quảng cáo (ad set)
    };
  }
  ...
  // Link bài quảng cáo: lấy ở cấp quảng cáo (ad), ưu tiên ad đang chạy
  try {
    ...
    const chosen = {}; // campaign_id -> {link, active}
    for (const ad of ads) {
      const cid = ad.campaign_id;
      if (!cid || !byId[cid]) continue;
      // Ưu tiên link tới bài đăng thật trên Facebook; chỉ dùng link preview khi không có
    ...
    // nếu không lấy được link thì bỏ qua, các số liệu khác vẫn hiển thị
  }

  // Nhóm quảng cáo (ad set): ngân sách + chi tiêu từng nhóm trong khoảng ngày
  try {
    const adsets = await fbAll(`act_${acc}/adsets`, {
    ...
    const asInsights = await fbAll(`act_${acc}/insights`, {
      level: 'adset',
    ...
    for (const id in asById) {
      ...
      // chỉ cộng ngân sách của nhóm ĐANG BẬT
      if (as.on) budgetSum[as.campaign_id] = (budgetSum[as.campaign_id] || 0) + as.budget;
    }
    // Nếu chiến dịch không đặt ngân sách ở cấp chiến dịch (ABO) → dùng tổng ngân sách các nhóm
    for (const cid in budgetSum) {
      if (byId[cid] && !byId[cid].budget) byId[cid].budget = budgetSum[cid];
    }
  } catch (e) {
    // nếu không lấy được nhóm thì bỏ qua, các số liệu khác vẫn hiển thị
  }
...
app.get('/api/data', async (req, res) => {
  try {
    ...
    if (me.role !== 'admin') {
      ...
      campaigns = campaigns.filter(c => allow.has(c.employee));
    }
    res.json({ days, campaigns, me: { user: me.user, role: me.role } });
  } catch (e) {
