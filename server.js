...
// ═══════════════════════════════════════════════════════════════════════════
//  THAILAND ORDERS — dán đoạn này vào server.js TRƯỚC dòng app.listen(...)
//  Không sửa gì code cũ. Chỉ thêm vào cuối.
// ═══════════════════════════════════════════════════════════════════════════

import mysql from 'mysql2/promise';

// ── Kết nối MySQL ──────────────────────────────────────────────────────────
// Thêm vào .env: TH_DB_HOST, TH_DB_USER, TH_DB_PASS, TH_DB_NAME
// Dùng DB riêng để không ảnh hưởng gì đến project cũ
const thPool = mysql.createPool({
  host:             process.env.TH_DB_HOST || 'localhost',
  user:             process.env.TH_DB_USER,
  password:         process.env.TH_DB_PASS,
...
(async () => {
  try {
    const conn = await thPool.getConnection();
    await conn.execute(`
      CREATE TABLE IF NOT EXISTS thailand_orders (
        ...
        status      VARCHAR(50)  DEFAULT 'Mới về',
        note        TEXT         DEFAULT '',
        source_page VARCHAR(255) DEFAULT ''
      ) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci
    `);
    conn.release();
    console.log('[Thailand] ✅ DB sẵn sàng');
  } catch (e) {
    console.error('[Thailand] ❌ Không kết nối được DB:', e.message);
  }
})();

// ── Parse combo → số lượng thực nhận + giá ────────────────────────────────
// Theo đúng công thức IFS bạn dùng trong Google Sheet
const TH_COMBO_MAP = {
  'คอมโบ 2 กล่อง: 549 THB + ค่าส่ง 49 THB':              { qty: 2, price: 598  },
  'คอมโบ 3 กล่อง: 799 THB (แถมฟรี 1 กล่อง + ส่งฟรี)':    { qty: 4, price: 799  },
  'คอมโบ 4 กล่อง: 959 THB (แถมฟรี 1 กล่อง + ส่งฟรี)':    { qty: 5, price: 959  },
  'คอมโบ 5 กล่อง: 1,129 THB (แถมฟรี 1 กล่อง + ส่งฟรี)':  { qty: 6, price: 1129 },
};
function thParseCombo(raw) {
  return TH_COMBO_MAP[(raw || '').trim()] || { qty: null, price: null };
}

// ── Middleware kiểm tra session cho /thailand/* ────────────────────────────
function thAuth(req, res, next) {
  // Dùng chung session store của project cũ — không cần cài thêm gì
  if (req.session && req.session.user) return next(); // admin cũ đăng nhập = có quyền
  if (req.session && req.session.thailand_user) return next(); // hoặc login riêng /thailand
  if (req.path.startsWith('/api/')) return res.status(401).json({ error: 'Chưa đăng nhập' });
  res.redirect('/thailand/login');
}

// ── Trang đăng nhập riêng cho /thailand (nếu không dùng account cũ) ───────
app.get('/thailand/login', (req, res) => {
  // Nếu đã đăng nhập qua system cũ → vào thẳng
  if (req.session && req.session.user) return res.redirect('/thailand/');
  res.send(`<!DOCTYPE html><html lang="vi"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Đơn Thái Lan — Đăng nhập</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:system-ui,sans-serif;background:#0f172a;color:#fff;
       display:grid;place-items:center;min-height:100vh}
  .box{background:#1e293b;border:1px solid rgba(255,255,255,.1);
       border-radius:16px;padding:32px 28px;width:320px}
  .flag{font-size:30px;margin-bottom:12px}
  h1{font-size:18px;margin-bottom:4px}
  p{color:#94a3b8;font-size:13px;margin-bottom:22px}
  ...
  button{width:100%;margin-top:18px;padding:11px;border:none;
         border-radius:8px;background:#3b82f6;color:#fff;
         font-size:14px;font-weight:600;cursor:pointer}
  button:hover{background:#2563eb}
  .err{background:#450a0a;color:#fca5a5;font-size:12px;
       padding:8px 12px;border-radius:7px;margin-top:12px}
</style></head><body>
<form class="box" method="POST" action="/thailand/login">
  <div class="flag">🇹🇭</div>
  <h1>TDM — Đơn Thái Lan</h1>
  <p>ads.tdmjsc.com/thailand</p>
  <label>Tên đăng nhập</label>
  <input name="user" autocomplete="username" required>
  <label>Mật khẩu</label>
  <input name="pass" type="password" autocomplete="current-password" required>
  ${req.query.error ? '<div class="err">Sai tên đăng nhập hoặc mật khẩu.</div>' : ''}
  <button type="submit">Đăng nhập</button>
</form></body></html>`);
});

app.post('/thailand/login', (req, res) => {
  const { user, pass } = req.body;
  const TH_USER = process.env.TH_ADMIN_USER || 'thailand';
  const TH_PASS = process.env.TH_ADMIN_PASS || 'doi-mat-khau-nay';
  if (user === TH_USER && pass === TH_PASS) {
    req.session.thailand_user = { user, role: 'admin' };
    return res.redirect('/thailand/');
  }
  res.redirect('/thailand/login?error=1');
});

app.get('/thailand/logout', (req, res) => {
  delete req.session.thailand_user;
  res.redirect('/thailand/login');
});

// ── Serve trang quản lý HTML ───────────────────────────────────────────────
// File đặt tại: /home/u422036594/domains/ads.tdmjsc.com/nodejs/public/thailand/index.html
app.use('/thailand', thAuth, express.static(
  path.join(__dirname, 'public/thailand')
));
app.get('/thailand/', thAuth, (req, res) => {
  res.sendFile(path.join(__dirname, 'public/thailand/index.html'));
});
app.get('/thailand', (req, res) => res.redirect('/thailand/'));

// ── WEBHOOK — Ladipage POST đơn về đây ────────────────────────────────────
// URL điền vào Ladipage: https://ads.tdmjsc.com/api/webhook/thailand
// Field map theo đúng cấu hình Ladipage của bạn (ảnh đã chụp):
//   name → họ tên    phone → SĐT    address → địa chỉ    message → combo
app.post('/api/webhook/thailand', async (req, res) => {
  try {
    const b = req.body;
    console.log('[Thailand Webhook]', JSON.stringify(b));

    const full_name   = (b.name     || '').trim();
    const phone       = (b.phone    || '').trim();
    const address     = (b.address  || '').trim();
    const combo_raw   = (b.message  || b.combo || b.product || '').trim();
    const marketer    = (b.marketer || b.marketing || '').trim();
    const source_page = (b.source   || req.headers['referer'] || '').trim();

    const { qty, price } = thParseCombo(combo_raw);

    await thPool.execute(
      `INSERT INTO thailand_orders
         (full_name, phone, address, combo_raw, qty, price, marketer, source_page)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [full_name, phone, address, combo_raw, qty, price, marketer, source_page]
    );

    console.log(`[Thailand] ✅ ${full_name} | ${phone} | ${combo_raw}`);
    res.json({ success: true });
  } catch (err) {
    console.error('[Thailand Webhook] ❌', err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});

// ── API: lấy danh sách đơn ────────────────────────────────────────────────
app.get('/api/thailand/orders', thAuth, async (req, res) => {
  try {
    let sql = 'SELECT * FROM thailand_orders WHERE 1=1';
    const params = [];
    if (req.query.status) { sql += ' AND status = ?'; params.push(req.query.status); }
    if (req.query.mk)     { sql += ' AND marketer = ?'; params.push(req.query.mk); }
    if (req.query.q) {
      sql += ' AND (full_name LIKE ? OR phone LIKE ? OR address LIKE ?)';
      const q = `%${req.query.q}%`;
      params.push(q, q, q);
    }
    sql += ' ORDER BY received_at DESC LIMIT 2000';
    const [rows] = await thPool.execute(sql, params);
...
// ── API: cập nhật trạng thái + ghi chú ───────────────────────────────────
app.put('/api/thailand/orders/:id', thAuth, async (req, res) => {
  try {
    const { status, note } = req.body;
    await thPool.execute(
      'UPDATE thailand_orders SET status = ?, note = ? WHERE id = ?',
      [status || 'Mới về', note || '', req.params.id]
    );
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
...
// ═══════════════════════════════════════════════════════════════════════════
//  KẾT THÚC THAILAND ORDERS — sau đây là app.listen(...) của project cũ
// ═══════════════════════════════════════════════════════════════════════════
