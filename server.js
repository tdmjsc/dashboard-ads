...
// Nhận diện đơn "giao thành công" theo tên trạng thái giao vận.
// (Tài liệu không cho mã cố định cho "giao thành công", nên dò theo tên.
//  Nếu hệ thống của bạn dùng chữ khác, sửa danh sách từ khoá dưới đây.)
function isShippedOK(o) {
  const s = (o.orderStatusName || '').toLowerCase();
  return s.includes('thành công') || s.includes('đã giao') || s.includes('giao thanh cong');
}
...
function aggregateMarketing(orders, acc) {
  for (const o of orders) {
    ...
    for (const d of items) {
      const ten = ((d.tenSanPham || d.productName || d.ten || d.name || '') + '').trim();
      if (!ten) continue;
      if (!a.products[ten]) a.products[ten] = { ten, soDon: 0, soLuong: 0 };
      a.products[ten].soLuong += Number(d.soLuong || d.quantity || 0);
...
async function refreshMarketing(since, until) {
  ...
  try {
    for (let page = 1; page <= MKT_MAX_PAGES; page++) {
      ...
      MKT.totalRecord = json.totalRecord || (MKT.loaded + orders.length);
      MKT.loaded += orders.length;
      MKT.rows = Object.values(acc).map(r => ({
        name: r.name, soDon: r.data, chot: r.chot, ship: r.ship, doanhthu: r.doanhthu,
        products: Object.values(r.products).sort((x, y) => y.soDon - x.soDon),
      })).sort((a, b) => (b.chot - a.chot) || (b.doanhthu - a.doanhthu)); // điền dần để xem tiến độ
      if (orders.length < 100 || MKT.loaded >= MKT.totalRecord) break;       // hết trang
      await sleep(MKT_PAGE_DELAY_MS);                                        // tôn trọng giới hạn 60s
    }
